...
const date = new Date();
const getMS = date.getMilliseconds();
...
test.describe('End to end test of creating an account', () =>{ 
    ...
    test('verify account does not exist', async ({ page }) => {
          ...
          await page.getByTestId('username-input').fill(`tester${getMS}`);
          await page.getByTestId('email-input').fill(`testing${getMS}@gmail.com`);
          await page.getByTestId('pwd-input').fill(String(getMS));
    ...
    test('create an account', async ({ page }) => {
        ...
        //get the input fields and click the sign in button
        const email = `testing${getMS}@gmail.com`;
        await page.getByTestId('username-input').fill(`tester${getMS}`);
